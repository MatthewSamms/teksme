...
public enum GatewayKind {
	...
	private static final GatewayKind[] VALUES_ARRAY = new GatewayKind[] {
			SERIAL_MODEM, CLICKATELL_HTTP_GATEWAY, DIGICEL_HTTP_GATEWAY, };

	public static final List<GatewayKind> VALUES = Collections
			.unmodifiableList(Arrays.asList(VALUES_ARRAY));

	public static GatewayKind get(String literal) {
		for (int i = 0; i < VALUES_ARRAY.length; ++i) {
			GatewayKind result = VALUES_ARRAY[i];
	 ...
	 * @generated
	 */
	public static GatewayKind getByName(String name) {
		for (int i = 0; i < VALUES_ARRAY.length; ++i) {
			GatewayKind result = VALUES_ARRAY[i];
	...
	private GatewayKind(int value, String name, String literal) {
		this.value = value;
		this.name = name;
