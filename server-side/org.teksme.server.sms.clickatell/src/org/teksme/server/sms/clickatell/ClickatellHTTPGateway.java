...
public class ClickatellHTTPGateway extends HTTPGateway {
	...
	private static Logger logger = Logger.getLogger(ClickatellHTTPGateway.class
			.getName());

	public ClickatellHTTPGateway(String id, String myApiId, String myUsername,
			String myPassword) {
		super(id);
		this.apiId = myApiId;
		...
		setAttributes(AGateway.GatewayAttributes.RECEIVE
				| AGateway.GatewayAttributes.SEND
				| AGateway.GatewayAttributes.WAPSI
	...
	public void startGateway() throws TimeoutException, GatewayException,
		...
		logger.info("Starting gateway: " + getGatewayId());
		connect();
		super.startGateway();
	...
	public void stopGateway() throws TimeoutException, GatewayException,
			IOException, InterruptedException {
		logger.info("Stoping gateway: " + getGatewayId());
	...
	@Override
	public boolean queryCoverage(OutboundMessage msg) throws TimeoutException,
		...
		if (response.get(0).indexOf("OK") == 0)
			return true;
		return false;
	}
	...
	public DeliveryStatuses queryMessage(String refNo) throws TimeoutException,
		...
		setDeliveryErrorCode(Integer.parseInt(response.get(0)
				.substring(pos + 7).trim()));
	...
	boolean authenticate() throws IOException, MalformedURLException {
		...
		List<HttpHeader> request = new ArrayList<HttpHeader>();
		List<String> response;
		logger.info("Authenticate() :: " + getGatewayId());
	...
	public boolean sendMessage(OutboundMessage msg) throws TimeoutException,
		...
		if (this.sessionId == null) {
			...
			logger.info("No session defined. " + getGatewayId());

			msg.setFailureCause(FailureCauses.GATEWAY_FAILURE);
			return false;
		}
		logger.info("sendMessage() ::" + getGatewayId());
		...
		try {
			if (msg.getType() == MessageTypes.OUTBOUND) {
				...
				logger.info("isInboud() ::" + this.isInbound());
				if (this.isInbound())
					request.add(new HttpHeader("mo", "1", false));
				...
				logger.info("Incorrect message format." + getGatewayId());

				return false;
			}
			request.add(new HttpHeader("session_id", this.sessionId, false));
			request.add(new HttpHeader("to", msg.getRecipient().substring(1),
					false));
			...
			if (msg.getFrom() != null && msg.getFrom().length() != 0)
				...
				request.add(new HttpHeader("from", getFrom(), false));
			if (msg.getFlashSms())
				request.add(new HttpHeader("msg_type", "SMS_FLASH", false));
			if (msg.getType() == MessageTypes.OUTBOUND) {
				if ((msg.getSrcPort() != -1) || (msg.getDstPort() != -1))
					request.add(new HttpHeader("udh", msg
							.getPduUserDataHeader(), false));
				...
				request.add(new HttpHeader("si_id",
						String.valueOf(msg.getId()), false));
				if (((OutboundWapSIMessage) msg).getCreateDate() != null)
					request.add(new HttpHeader("si_created",
							formatDateUTC(((OutboundWapSIMessage) msg)
									.getCreateDate()), false));
				if (((OutboundWapSIMessage) msg).getExpireDate() != null)
					request.add(new HttpHeader("si_expires",
							formatDateUTC(((OutboundWapSIMessage) msg)
									.getExpireDate()), false));
				request.add(new HttpHeader("si_action",
						formatSignal(((OutboundWapSIMessage) msg).getSignal()),
						false));
				request.add(new HttpHeader("si_url",
						((OutboundWapSIMessage) msg).getUrl().toString(), false));
				request.add(new HttpHeader("si_text",
						((OutboundWapSIMessage) msg).getIndicationText(), false));
			}
			if (msg.getStatusReport())
				request.add(new HttpHeader("deliv_ack", "1", false));
			if ((getFrom() != null && getFrom().length() != 0)
					|| (msg.getFrom() != null && msg.getFrom().length() != 0))
	...
	String formatSignal(WapSISignals signal) {
		if (signal == WapSISignals.NONE)
			return "signal-none";
		else if (signal == WapSISignals.LOW)
			return "signal-low";
		else if (signal == WapSISignals.MEDIUM)
			return "signal-medium";
		else if (signal == WapSISignals.HIGH)
			return "signal-high";
		else if (signal == WapSISignals.DELETE)
